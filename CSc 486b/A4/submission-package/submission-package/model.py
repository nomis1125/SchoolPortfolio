...
class MyNetwork(nn.Module):
    ...
    def __init__(self, config, input_shp, mean=[None], std=[None]):#Changed bc if statements would not run
        ...
        if std.any() == None:
            std = torch.ones((50000, 324), dtype = torch.float32)#TODO
        else:
            std = torch.tensor(std)
        if mean.any() == None:
            mean = torch.zeros((50000, 324), dtype = torch.float32)#TODO
        else:
        ...
        self.std = torch.nn.Parameter(data = std, requires_grad = False)
        ...
        for i in range(0, config.num_hidden):
            self.relu[i] = setattr(self, self.ReLU, i)#TODO
            self.linear[i] = setattr(self, self.linear, i)
