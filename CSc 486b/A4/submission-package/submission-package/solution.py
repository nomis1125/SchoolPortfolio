...
def train(config):
    ...
    train_data = torch.utils.data.TensorDataset(x, y)#TODO
    valid_data = torch.utils.data.TensorDataset(x, y)#TODO
    ...
    tr_data_loader = DataLoader(train_data, batch_size=8, num_workers=1)#TODO
    va_data_loader = DataLoader(valid_data, batch_size=8, num_workers=1)#TODO

    # TODO (1 point): Create model instance. We will use
    # `train_data.sample_shp`, mean and std from training data for
    # initialization.
    model = MyNetwork(config = config, input_shp = tr_data_loader, mean = x_tr_mean, std = x_tr_std)#TODO
    ...
    for epoch in range(config.num_epoch):
        ...
        for data in tqdm(tr_data_loader, desc=prefix):
            ...
            scoreX = model.forward(self, x)
            scoreY = model.forward(self, y)

            Total_Loss = nn.L1Loss()(data_loss, model_loss)#TODO
            ...
            if iter_idx % config.rep_intv == 0:
                ...
                # epoch.
                torch.save(model.state_dict(), checkpoint_file)#TODO
            ...
            if iter_idx % config.val_intv == 0:
                ...
                for data in va_data_loader:
                    ...
                    scoreX = model.forward(self, x)
                    scoreY = model.forward(self, y)#TODO
...
def test(config):
    ...
    test_data = torch.utils.data.TensorDataset(x, y)#TODO
    ...
    # shuffling.
    te_data_loader = DataLoader(train_data, batch_size=8, num_workers=2)#TODO

    # Create model
    model = MyNetwork(config = config, input_shp = te_data_loader, mean = x_tr_mean, std = x_tr_std)#TODO
    ...
    model.load_state_dict(checkpoint_file)#TODO
    ...
    for data in tqdm(te_data_loader, desc=prefix):
        ...
        #NOTE: Seems like this should be done in a similar manner as the training but instead of storing the information into self we just store what we get from each accuracy and loss into te_acc[] and te_loss[].
        #TODO
