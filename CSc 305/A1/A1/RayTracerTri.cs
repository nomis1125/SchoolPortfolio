...
public class RayTracerTri : MonoBehaviour {
			...
			for(int j = 0; j < pixel_height; ++j){
				...
				if(nd != 0){
					...
					if(Vector3.Dot(N, Vector3.Cross(edge0, C0)) > 0 && Vector3.Dot(N, Vector3.Cross(edge1, C1)) > 0 && Vector3.Dot(N, Vector3.Cross(edge2, C2)) > 0){
						//inside triangle;
						RayTracingResult.SetPixel(i, j, new Color(Value1, Value2, Value3));
					}
				}
